...
WITH ELEGIBILIDAD AS (
  SELECT C.CAM_CAMPAIGN_ID CAMPAIGN_ID,
    ...
    CASE WHEN C.CAM_CAMPAIGN_BENEFITS[SAFE_OFFSET(0)].TYPE = 'VOLUME' THEN C.CAM_CAMPAIGN_BENEFITS[SAFE_OFFSET(0)].CONFIGS[SAFE_OFFSET(0)].DISCOUNT_VOLUME.MELI_DISCOUNT_COVERAGE
        ELSE C.CAM_CAMPAIGN_BENEFITS[SAFE_OFFSET(0)].CONFIGS[SAFE_OFFSET(0)].REBATE_MELI_PERCENTAGE END AS MELI_PERCENTAGE,
    ...
    E.SIT_SITE_ID,
    E.ITE_ITEM_ID,
        -- Los DxV pueden optinear antes de que inicie la campaña, entonces se agrega el status NOT_STARTED_YET para atajarse a esto
    MIN(CASE WHEN S.DIS_TYPE = 'VOLUME' AND S.DIS_STATUS_REASON = 'NOT_STARTED_YET' AND TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 4 HOUR) < CAM_CAMPAIGN_START_DTTM THEN 'NOT_STARTED_YET'
        -- Los DxI no se rigen por fechas, sino que por los estados
        WHEN S.DIS_TYPE = 'PRE_NEGOTIATED' AND S.DIS_STATUS_ID = 'ACTIVE' THEN 'ACTIVE'
        WHEN S.DIS_TYPE = 'PRE_NEGOTIATED' AND S.DIS_STATUS_REASON = 'NOT_STARTED_YET' THEN 'NOT_STARTED_YET'
        WHEN S.DIS_TYPE = 'PRE_NEGOTIATED' THEN 'ELIGIBLE'
        -- El resto de los rebates se rigen unicamente por fechas de inicio y fin
        WHEN TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 4 HOUR) BETWEEN s.DIS_START_DATE_DTTM AND S.DIS_END_DATE_DTTM THEN 'ACTIVE' 
        ELSE 'ELIGIBLE' END) AS ELIGIBILITY_STATUS,
    MAX(CASE WHEN S.DIS_STATUS_ID = 'ACTIVE'
        OR (S.DIS_STATUS_ID = 'INACTIVE' AND S.DIS_STATUS_REASON != 'NOT_STARTED_YET') THEN 1 ELSE 0 END) HAD_REBATE,
